...
export class SonarrGetterService {
  ...
  async get(
    ...
    try {
      ...
      let episodeFilePromise: Promise<SonarrEpisodeFile | undefined> | undefined
      ...
      switch (prop.name) {
        ...
        case 'seasonFinale': {
          ...
          return episodes.some((el) => el.finaleType === 'season' && el.hasFile)
        }
        case 'seriesFinale': {
          const episodes = await sonarrApiClient.getEpisodes(
            showResponse.id,
          ...
          return episodes.some((el) => el.finaleType === 'series' && el.hasFile)
        }
        case 'seasonNumber': {
