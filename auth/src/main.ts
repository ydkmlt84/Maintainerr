...
async function bootstrap() {
    ...
    logger: WinstonModule.createLogger({
      ...
      format: winston.format.combine(
        ...
        winston.format.printf(({ level, message, timestamp, context }) => {
          ...
          return `${chalk.green(
            `[maintainerr_auth] |`
          )} ${coloredTimestamp}  ${coloredLevel} ${chalk.blue(
            `[${context}]`
          )} ${coloredMessage}`
        })
      ),
      transports: [
        ...
        new DailyRotateFile({
          ...
          format: winston.format.combine(
            winston.format.timestamp({ format: 'DD/MM/YYYY HH:mm:ss' }),
            winston.format.printf(({ level, message, timestamp, context }) => {
              return `[maintainerr_auth]  |  ${timestamp}  [${level.toUpperCase()}] [${context}] ${message}`
            })
          ),
  ...
  // Start the server on port 3005 or an environment-defined port
  const authPort = process.env.AUTH_PORT || 3005
